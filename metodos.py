...
def aux_euler_inverso(expr,t0 ,y0 ,h, n):
	...
	v_Aux.append([y, t])
	for i in range(n):
		k = y + h*f_calculate(expr, y, t)
		y = y + h*f_calculate(expr, k, t+h) # formula tradicional do metodo de euler
...
def aux_euler_aprimorado(expr, t0, y0, h, n):
    ...
    for i in range(n):
	    k = y + h*f_calculate(expr,y,t)
	    y = y + h/2*(f_calculate(expr,y,t)+f_calculate(expr, k, t+h)) # formula tradicional do metodo de euler
	    t = t + h # incrementa o tn para a proxima iteracao
	    v_Aux.append([y,t])
    return
...
def aux_runge_kutta(expre,t0,y0,h,n):
    ...
    v_Aux.append([y,t])
    for i in range(0,n):
	    k1 = f_calculate(expre, y, t)
	    k2 = f_calculate(expre, y +(h/2)*k1, t + h/2)
	    k3 = f_calculate(expre, y + (h/2)*k2, t + h/2)
	    k4 = f_calculate(expre, y + h*k3, t + h)
...
def Adams__Bash(metodo, expre, y0, t0, h, n, grau):
...
def AdamBashF(expre, h, g, i):
	fn1 = f_calculate(expre, v_Aux[i][0], v_Aux[i][1])
	fn2 = f_calculate(expre, v_Aux[i-1][0], v_Aux[i-1][1])
	if(g == 2):
		res = (h/2)*(3*fn1 - fn2)
	elif(g == 3):
		fn3 = f_calculate(expre, v_Aux[i-2][0], v_Aux[i-2][1])
		res = (h/12)*(23*fn1 - 16*fn2 + 5*fn3)
	elif(g == 4):
		fn3 = f_calculate(expre, v_Aux[i-2][0], v_Aux[i-2][1])
		fn4 = f_calculate(expre, v_Aux[i-3][0], v_Aux[i-3][1])
		res = (h/24)*(55*fn1 - 59*fn2 + 37*fn3 - 9*fn4)
	elif(g == 5):
		fn3 = f_calculate(expre, v_Aux[i-2][0], v_Aux[i-2][1])
		fn4 = f_calculate(expre, v_Aux[i-3][0], v_Aux[i-3][1])
		fn5 = f_calculate(expre, v_Aux[i-4][0], v_Aux[i-4][1])
		res = (h/720)*(1901*fn1 - 2774*fn2 + 2616*fn3 - 1274*fn4 + 251*fn5)
	elif(g == 6):
		fn3 = f_calculate(expre, v_Aux[i-2][0], v_Aux[i-2][1])
		fn4 = f_calculate(expre, v_Aux[i-3][0], v_Aux[i-3][1])
		fn5 = f_calculate(expre, v_Aux[i-4][0], v_Aux[i-4][1])
		fn6 = f_calculate(expre, v_Aux[i-5][0], v_Aux[i-5][1])
		res = (h/1440)*(4277*fn1 - 7923*fn2 + 9982*fn3 - 7298*fn4 + 2877*fn5 - 475*fn6)
	return res

def FAdamsMoulton(yf, expre, h, g, i):
	fn1 = f_calculate(expre, y, v_Aux[i][1]+h)
	fn2 = f_calculate(expre, v_Aux[i][0], v_Aux[i][1])
	if(g == 2):
		aux = yf + (h/2)*(fn1 + fn2)
	elif(g == 3):
		fn3 = f_calculate(expre, v_Aux[i-1][0], v_Aux[i-1][1])
		aux = (h/12)*(5*fn1 + 8*fn2 - fn3)
	elif(g == 4):
		fn3 = f_calculate(expre, v_Aux[i-1][0], v_Aux[i-1][1])
		fn4 = f_calculate(expre, v_Aux[i-2][0], v_Aux[i-2][1])
		aux = yf + (h/24)*(9*fn1 + 19*fn2 - 5*fn3 + 1*fn4)
	elif(g == 5):
		fn3 = f_calculate(expre, v_Aux[i-1][0], v_Aux[i-1][1])
		fn4 = f_calculate(expre, v_Aux[i-2][0], v_Aux[i-2][1])
		fn5 = f_calculate(expre, v_Aux[i-3][0], v_Aux[i-3][1])
		aux = yf + (h/720)*(251*fn1 + 646*fn2 - 264*fn3 + 106*fn4 - 19*fn5)
	elif(g == 6):
		fn3 = f_calculate(expre, v_Aux[i-1][0], v_Aux[i-1][1])
		fn4 = f_calculate(expre, v_Aux[i-2][0], v_Aux[i-2][1])
		fn5 = f_calculate(expre, v_Aux[i-3][0], v_Aux[i-3][1])
		fn6 = f_calculate(expre, v_Aux[i-4][0], v_Aux[i-4][1])
		aux = yf + (h/1440)*(475*fn1 + 1427*fn2 - 798*fn3 + 482*fn4 - 173*fn5 + 27*fn6)
	elif(g == 7):
		fn3 = f_calculate(expr, v_Aux[i-1][0], v_Aux[i-1][1])
		fn4 = f_calculate(expr, v_Aux[i-2][0], v_Aux[i-2][1])
		fn5 = f_calculate(expr, v_Aux[i-3][0], v_Aux[i-3][1])    	
		fn6 = f_calculate(expr, v_Aux[i-4][0], v_Aux[i-4][1])
		fn7 = f_calculate(expr, v_Aux[i-5][0], v_Aux[i-5][1])
		...
		fn6 = f_calculate(expr, v_Aux[i-4][0], v_Aux[i-4][1])
		fn7 = f_calculate(expr, v_Aux[i-5][0], v_Aux[i-5][1])
		fn8 = f_calculate(expr, v_Aux[i-6][0], v_Aux[i-6][1])
...
def init(entradas):# function that chooses which method will be called
	...
	if (entradas[0] == 'euler'):
		...
		if (grau == 2):
			Adams__Bash(entradas[0], expr, y0, t0, h, n, grau-1)
		elif (grau == 3):
		...
		n = int(entradas[4])
		grau = int(entradas[6])
		Adams__Bash(entradas[0], expr, y0, t0, h, n, grau)
		...
		v_Aux.append([float(entradas[1]),t0]) # primeiro ponto
		if (grau == 2):
			Adams__Multon(entradas[0], expr, y0, t0, h, n, grau-1)
		elif (grau == 3):
			v_Aux.append([float(entradas[2]),t0+h]) # segundo ponto
			Adams__Multon(entradas[0], expr, y0, t0, h, n, grau-1)
		elif (grau == 4):
		...
		grau = int(entradas[6])
		tf = (t0 + (h*n))
		Adams__Multon(entradas[0], expr, y0, t0, h, n, grau)
		...
		v_Aux.append([float(entradas[1]),t0]) # primeiro ponto
		if (grau == 2):
			FInversa(entradas[0], expr, y0, t0, h, n, grau-1)
		elif (grau == 3):
			v_Aux.append([float(entradas[2]),t0+h]) # segundo ponto
			FInversa(entradas[0], expr, y0, t0, h, n, grau-1)
		elif (grau == 4):
